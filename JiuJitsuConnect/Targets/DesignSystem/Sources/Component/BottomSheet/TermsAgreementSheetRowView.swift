...
public struct TermsAgreementRow: View {
    ...
    public init(title: String, typeText: String, isChecked: Bool, leadingIconColor: Color, labelColor: Color, typeTextColor: Color, followingIconColor: Color, onCheckTapped: @escaping () -> Void, onSeeDetailsTapped: (() -> Void)? = nil) {
        self.title = title
    ...
    public var body: some View {
        HStack(spacing: 0) {
            ...
            // 상세보기 버튼 (contentURL 이 있는 항목에만 표시)
            if let onSeeDetailsTapped {
                Button(action: onSeeDetailsTapped) {
